% Script to find and plot optimal wavelets for Kuramoto-Sivashinsky data. 
% Roughly reproduces figure 5 in the paper. 

% Load Kuramoto-Sivashinsky dataset
load('data/ksdataChaotic.mat')
Z = udata(:,10001:end);
[m,n] = size(Z);

% Set some parameters
levels = 6; % = log2(m)
lambda2 = 0.1; % variance penalty
...
% Plot the wavelets we computed, offset from each other by 0.5, as well as
% their power spectra
...
for j=1:levels+1
    ...
    figure(1)
    hold on
    plot([0:m-1],(j-1)*0.5 + Zr,'-','color',cols(j,:),'linewidth',2)
